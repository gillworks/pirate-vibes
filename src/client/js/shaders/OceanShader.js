...
const OceanShader = {
  ...
  vertexShader: `
    ...
    void main() {
      ...
      // Large waves - increased amplitude for more dramatic effect
      newPosition.y += generateWave(position.xz, time, 60.0, waveSpeed, waveDirection) * 2.0;
      
      // Medium waves - more pronounced
      newPosition.y += generateWave(position.xz, time * 1.3, 30.0, waveSpeed * 0.8, 
                                   vec2(waveDirection.y, -waveDirection.x)) * 1.2;
      
      // Small waves/ripples
      newPosition.y += generateWave(position.xz, time * 2.0, 15.0, waveSpeed * 1.2, 
                                   vec2(-waveDirection.x, -waveDirection.y)) * 0.6;
      
      // Very small ripples
      newPosition.y += generateWave(position.xz, time * 3.0, 5.0, waveSpeed * 1.5, 
                                   vec2(-waveDirection.y, waveDirection.x)) * 0.2;
      ...
      nearbyX.y = position.y + generateWave(nearbyX.xz, time, 60.0, waveSpeed, waveDirection) * 2.0;
      
      vec3 nearbyZ = newPosition + vec3(0.0, 0.0, 0.1);
      nearbyZ.y = position.y + generateWave(nearbyZ.xz, time, 60.0, waveSpeed, waveDirection) * 2.0;
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      float sunSpecular = pow(max(0.0, dot(reflectionDir, sunDirection)), shininess) * 0.8;
      
      // Calculate foam based on wave height and normal
      float foam = pow(max(0.0, normal.y), 5.0) * 0.8;
      foam += pow(max(0.0, sin(vPosition.x * 0.1 + time) * sin(vPosition.z * 0.1 + time * 0.8)), 2.0) * 0.5;
      
      // Add more dramatic foam patterns
      foam += 0.2 * sin(vPosition.x * 0.05 + time * 0.5) * sin(vPosition.z * 0.05 + time * 0.7);
      ...
      waterColorMixed += vec3(sin(time * 0.1) * 0.05, sin(time * 0.11) * 0.05, sin(time * 0.12) * 0.1);
  ...
  uniforms: {
    ...
    waterColor: { value: new THREE.Color(0x0088ff) }, // Brighter blue
